...
interface TradieReviewsResponse {
  data: Review[]
  ...
  const fetchReviews = useCallback(async () => {
    try {
      const res = await api.raw<TradieReviewsResponse>(
        `/api/reviews/tradie/${tradieId}?limit=50`,
        { noAuth: true }
      )
      setReviews(res.data)
      setStats(res.stats)
  ...
  if (!user || !profile) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-4">
        ...
        <Link href="/find-talent" className="text-sm text-(--upwork-green) hover:underline mt-2">
          Browse all tradies
        </Link>
      </div>
    )
    ...
    <div className="min-h-screen bg-[#f9faf9]">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-3">
          <Link
            href="/find-talent"
            className="flex items-center gap-1.5 text-sm text-(--upwork-gray) hover:text-(--upwork-navy) transition-colors w-fit"
          >
